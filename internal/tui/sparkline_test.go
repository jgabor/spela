...
// ---------------------------------------------------------------------------
// normalizeAndClamp tests (unit coverage for the thermal helper)
// ---------------------------------------------------------------------------
...
func TestNormalizeAndClamp(t *testing.T) {
	tests := []struct {
		...
		{"mid-range", 50, 0, 100, 0.5},
		{"at-min", 0, 0, 100, 0.0},
		{"at-max", 100, 0, 100, 1.0},
		{"below-min", -10, 0, 100, 0.0},
		{"above-max", 200, 0, 100, 1.0},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if got != tt.wantResult {
				t.Errorf("normalizeAndClamp(%f, %f, %f) = %f, want %f",
					tt.value, tt.min, tt.max, got, tt.wantResult)
			}
		})
	}
}
